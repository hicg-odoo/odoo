...
The helper will create `.env` automatically from `.env.example` or `env.example`.
...
- Windows Docker pipe error?
  ...
  - Start Docker Desktop, switch to **Linux containers**, and wait for **Engine running**.
- Build fails with `externally-managed-environment`?
  - Pull latest changes and rebuild (`./deploy.sh quickstart` or `.\deploy.ps1 quickstart`).
  - MCP now runs in a dedicated Python container (no pip install in Odoo image).
