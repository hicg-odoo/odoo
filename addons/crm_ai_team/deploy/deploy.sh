#!/usr/bin/env bash
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
COMPOSE_FILE="$ROOT_DIR/docker-compose.yml"
ENV_FILE="$ROOT_DIR/.env"
ENV_EXAMPLE_DOT="$ROOT_DIR/.env.example"
ENV_EXAMPLE_PLAIN="$ROOT_DIR/env.example"

ensure_env() {
  if [[ ! -f "$ENV_FILE" ]]; then
    if [[ -f "$ENV_EXAMPLE_DOT" ]]; then
      cp "$ENV_EXAMPLE_DOT" "$ENV_FILE"
      echo "Created $ENV_FILE from .env.example"
    elif [[ -f "$ENV_EXAMPLE_PLAIN" ]]; then
      cp "$ENV_EXAMPLE_PLAIN" "$ENV_FILE"
      echo "Created $ENV_FILE from env.example"
    else
      echo "Missing env template. Add .env.example or env.example in $ROOT_DIR" >&2
      exit 1
    fi
  fi
}

compose() {
  docker compose --env-file "$ENV_FILE" -f "$COMPOSE_FILE" "$@"
}

ensure_docker() {
  if ! command -v docker >/dev/null 2>&1; then
    echo "Docker CLI is not installed or not in PATH." >&2
    exit 1
  fi
  if ! docker info >/dev/null 2>&1; then
    cat >&2 <<'MSG'
Cannot connect to the Docker engine.

If you are on Windows and see errors about:
  //./pipe/dockerDesktopLinuxEngine

Then Docker Desktop is likely not running (or Linux engine is unavailable).
Fix:
  1) Start Docker Desktop
  2) Switch to Linux containers
  3) Wait until Docker Desktop shows "Engine running"
MSG
    exit 1
  fi
}

cmd="${1:-quickstart}"
shift || true

case "$cmd" in
  quickstart)
    ensure_env
    ensure_docker
    compose up -d --build db odoo
    source "$ENV_FILE"
    echo "\nOdoo is starting. Open: http://localhost:${ODOO_HTTP_PORT:-8069}"
    echo "Tip: run './deploy.sh logs odoo' to follow startup logs."
    ;;
  up)
    ensure_env
    ensure_docker
    compose up -d --build "$@"
    ;;
  up-mcp)
    ensure_env
    ensure_docker
    compose --profile mcp up -d --build "$@"
    ;;
  seed-demo)
    ensure_env
    ensure_docker
    source "$ENV_FILE"
    compose exec -T odoo odoo shell \
      --db_host=db \
      --db_user="${POSTGRES_USER:-odoo}" \
      --db_password="${POSTGRES_PASSWORD:-odoo}" \
      -d "${ODOO_DB:-odoo}" <<'PY'
result = env['crm.ai.agent.team'].create_dummy_dataset(team_name='Local Demo Team', agent_count=5, transcript_count=15, auto_run=True)
print(result)
PY
    ;;
  down)
    ensure_env
    ensure_docker
    compose down "$@"
    ;;
  logs)
    ensure_env
    ensure_docker
    compose logs -f "$@"
    ;;
  ps)
    ensure_env
    ensure_docker
    compose ps "$@"
    ;;
  restart)
    ensure_env
    ensure_docker
    compose restart "$@"
    ;;
  *)
    echo "Usage: $0 {quickstart|up|up-mcp|seed-demo|down|logs|ps|restart} [args...]" >&2
    exit 1
    ;;
esac
