...
- `odoo/Dockerfile`: Odoo image with `crm_ai_team` only (no pip package install step).
- `mcp/Dockerfile`: dedicated MCP service image (`python:3.11-slim`) with `mcp` installed.
...
- `env.example`: same template without a leading dot (easier to spot in some UIs).
...
The deploy scripts auto-create `.env` from either `.env.example` or `env.example`.
...
## Common Windows Docker error

If Docker returns an error like:

`open //./pipe/dockerDesktopLinuxEngine: The system cannot find the file specified`

it usually means Docker Desktop (Linux engine) is not running.

Fix steps:
1. Start Docker Desktop.
2. Switch to **Linux containers**.
3. Wait until status shows **Engine running**.
4. Re-run `./deploy.sh quickstart` (or `.\deploy.ps1 quickstart`).

## Build error: `externally-managed-environment`

If you previously saw this during image build:

`error: externally-managed-environment`

that is Python's PEP 668 protection. To avoid this class of failure in Odoo
base images, MCP is now built in a separate Python container (`mcp/Dockerfile`)
instead of being installed in the Odoo image.
